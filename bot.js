...
client.on('message',  message => {
  if(bannedwords.some(word => message.content.includes(word))) {
    message.reply(" احترم نفسك ممنوع الشتيمة بدل ما تبلع ميوت ").then(msg => {msg.delete(5000)});;
  };
});
          ...
          .then(co => {
            text = co.first().content
              ...
              message.channel.send(`تم حفظ اقتراحك الرجاء انتضار الرد من قبل الاداره`)
                client.channels.get("532464781047758866").send(`${message.author.username}'s sug => ${text}`)
...
client.on('message', message => {
    ...
    .then(invites => message.channel.send(`لقد دعوت  ${invites.find(invite => invite.inviter.id === message.author.id).uses} عضو`))
...
const x5bz4 = [
 ...
 if (message.content.startsWith('&مريم')) {
  ...
  .setImage("https://a.top4top.net/p_1102b1b691.jpg")
...
client.on('message', msg => {
  ...
  let args = msg.content.split(" ").slice(1);
    ...
    if (textxt == "") {
        msg.delete().then
    msg.channel.send("***``` ضع عدد الرسائل التي تريد مسحها اقصي عدد هو 100 رساله 👌```***").then(m => m.delete(30000));
} else {
    msg.delete().then
    msg.delete().then
...
client.on('message', message => {
  ...
  if (message.mentions.users.size < 1) return message.reply("قم بعمل منشن للشخص و اترك مسافة و اكتب سبب الكيك");
...
const secreT = [
 ...
 client.on('message', message => {
   if (message.content.startsWith("d&minecraft")) {
                if(!message.channel.guild) return message.reply('** This command only for servers**');
  var embed = new Discord.RichEmbed()
  .setColor('RANDOM')
   .setThumbnail(message.author.avatarURL) 
